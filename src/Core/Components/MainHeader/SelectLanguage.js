...
import { useUpdateLangInfo, useLangInfo, ACTIONS } from 'src/Core/Context/LoginInfoContext'
...
const LanguageSelector = (props) => {
  ...
  return (
    <>
      ...
      >
        {!!LangInfo ? <>{languages.filter(l => {
          return l.alt === LangInfo;
        })[0].src}</> : <>{languages[0].src}</>}
      </IconButtonStyle>
      ...
      >
        {languages.map((el) => (
          <StyledMenuItem key={el.alt} value={el.alt} onClick={(e) => {
            console.log('click'); console.log(el.alt);
            langUpdate({type: ACTIONS.CHNG_LANG, payload: { langInfo: el.alt } });
            props.onClose(e)
          }}>
            {el.src}
